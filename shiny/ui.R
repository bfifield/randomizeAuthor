shinyUI(fluidPage(
    ...
    sidebarLayout(
        ...
        mainPanel(
            ...
            h3("Instructions:"),
            HTML("<ul><li>Input email addresses of at least one recipient under <strong>Recipient Emails</strong>. Separate each email with a semicolon or a comma. For example, <em>authorA@univ.edu; authorB@coll.edu</em>. A copy of the randomization results will also be sent to <em>randomizeauthor@gmail.com</em>.</li><li>Input author names in blank fields under <strong>Author Names</strong>.</li><li>Hit the <strong>Randomize</strong> button below, which will return the randomized author ordering and the random seed to replicate the results. Results will be emailed to the provided email addresses from <em>randomizeauthor@gmail.com</em>.</li><li>To replicate results, input author names in the same order, input the random seed in <strong>Random Seed</strong>, and hit the <strong>Randomize</strong> button below.</li></ul>"),
